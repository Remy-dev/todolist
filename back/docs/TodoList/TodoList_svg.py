#!/usr/bin/env python
# encoding: utf-8
# Généré par Mocodo 2.3.7 le Thu, 09 Jul 2020 08:35:29
...
with codecs.open('TodoList_geo.json') as f:
...
(ex,ey) = (cx[u"TASKS"],cy[u"TASKS"])
leg=straight_leg_factory(ex,ey,39,42,x,y,47,25,22+2*card_margin,15+2*card_margin)
...
(tx,ty)=offset(*leg.card_pos(False,shift[u"PEUT AVOIR,TASKS"]))
...
path=arrow(*leg.arrow_pos(">",ratio[u"PEUT AVOIR,TASKS"]))
...
leg=straight_leg_factory(ex,ey,46,34,x,y,47,25,21+2*card_margin,15+2*card_margin)
lines += u"""\n<line x1="%(ex)s" y1="%(ey)s" x2="%(ax)s" y2="%(ay)s" stroke="%(stroke_color)s" stroke-width="2"/>""" % {'ex': ex, 'ey': ey, 'ax': x, 'ay': y, 'stroke_color': colors['leg_stroke_color']}
(tx,ty)=offset(*leg.card_pos(False,shift[u"PEUT AVOIR,CATEGORIES"]))
...
path=arrow(*leg.arrow_pos(">",ratio[u"PEUT AVOIR,CATEGORIES"]))
lines += u"""\n<path d="%(path)s" fill="%(stroke_color)s" stroke-width="0"/>""" % {'path': path, 'stroke_color': colors['leg_stroke_color']}
...
lines += u"""\n	<text x="%(x)s" y="%(y)s" fill="%(text_color)s" font-family="Verdana" font-size="12">PEUT AVOIR</text>""" % {'x': -40+x, 'y': -7.3+y, 'text_color': colors['association_cartouche_text_color']}
lines += u"""\n</g>""" % {}

lines += u"""\n\n<!-- Entity TASKS -->"""
(x,y) = (cx[u"TASKS"],cy[u"TASKS"])
lines += u"""\n<g id="entity-TASKS">""" % {}
lines += u"""\n	<g id="frame-TASKS">""" % {}
...
lines += u"""\n	</g>""" % {}
lines += u"""\n	<text x="%(x)s" y="%(y)s" fill="%(text_color)s" font-family="Verdana" font-size="12">TASKS</text>""" % {'x': -21+x, 'y': -24.3+y, 'text_color': colors['entity_cartouche_text_color']}
...
lines += u"""\n	<text x="%(x)s" y="%(y)s" fill="%(text_color)s" font-family="Verdana" font-size="12">completion</text>""" % {'x': -34+x, 'y': 17.8+y, 'text_color': colors['entity_attribute_text_color']}
...
lines += u"""\n	</g>""" % {}
lines += u"""\n	<text x="%(x)s" y="%(y)s" fill="%(text_color)s" font-family="Verdana" font-size="12">CATEGORIES</text>""" % {'x': -41+x, 'y': -16.3+y, 'text_color': colors['entity_cartouche_text_color']}
lines += u"""\n	<text x="%(x)s" y="%(y)s" fill="%(text_color)s" font-family="Verdana" font-size="12">name</text>""" % {'x': -41+x, 'y': 8.8+y, 'text_color': colors['entity_attribute_text_color']}
lines += u"""\n	<text x="%(x)s" y="%(y)s" fill="%(text_color)s" font-family="Verdana" font-size="12">status</text>""" % {'x': -41+x, 'y': 25.8+y, 'text_color': colors['entity_attribute_text_color']}
lines += u"""\n</g>""" % {}
lines += u'\n</svg>'

with codecs.open("TodoList.svg", "w", "utf8") as f:
    f.write(lines)
safe_print_for_PHP(u'Fichier de sortie "TodoList.svg" généré avec succès.')